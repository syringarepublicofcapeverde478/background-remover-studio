...
:open_assistant
if defined PYW_EXE (
    start "" "%PYW_EXE%" "%~dp0src\setup_assistant.pyw"
) else (
    start "" "%PY_EXE%" "%~dp0src\setup_assistant.pyw"
)
exit /b 0
...
if not defined PYW_EXE (
    for /f "delims=" %%I in ('dir /b /s "%ProgramFiles%\Python*\pythonw.exe" 2^>nul') do (
        if not defined PYW_EXE set "PYW_EXE=%%I"
    )
)
if not defined PYW_EXE (
    for /f "delims=" %%I in ('dir /b /s "%ProgramFiles(x86)%\Python*\pythonw.exe" 2^>nul') do (
        if not defined PYW_EXE set "PYW_EXE=%%I"
    )
)
exit /b 0
